...
def profile(request):
    ...
    if request.method == 'POST':
        ...
        validate_default_street_address1 = (request.POST
                                            ['default_street_address1'])
        validate_default_street_address2 = (request.POST
        ...
        if not re.match("^[a-zA-Z ]+$", ''.join(validate_default_full_name)):
            messages.success(request, mark_safe('There was a problem with \
                default_full_name <br> Please try again.'))
            return redirect(reverse('home'))
        if not re.match("^[0-9]+$", ''.join(validate_default_phone_number)):
            messages.success(request, mark_safe('There was a problem with \
                default_phone_number <br> Please try again.'))
            return redirect(reverse('home'))
        if not re.match("^[a-zA-Z ]+$", ''
                        .join(validate_default_country) + "a"):
            messages.success(request, mark_safe('There was a problem with \
                default_country <br> Please try again.'))
            return redirect(reverse('home'))
        if not re.match("^[a-zA-Z0-9 ]+$", ''.join(validate_default_postcode)):
            messages.success(request, mark_safe('There was a problem with \
                default_postcode <br> Please try again.'))
            return redirect(reverse('home'))
        if not re.match("^[a-zA-Z ]+$",  ''
                        .join(validate_default_town_or_city)):
            messages.success(request, mark_safe('There was a problem with \
                default_town_or_city<br> Please try again.'))
            return redirect(reverse('home'))
        if not re.match("^[a-zA-Z0-9 ]+$", ''
                        .join(validate_default_street_address1)):
            messages.success(request, mark_safe('There was a problem with \
                default_street_address1 <br> Please try again.'))
            return redirect(reverse('home'))
        if not re.match("^[a-zA-Z0-9 ]+$", ''
                        .join(validate_default_street_address2) + "a"):
            messages.success(request, mark_safe('There was a problem with \
                default_street_address2 <br> Please try again.'))
            return redirect(reverse('home'))
        if not re.match("^[a-zA-Z ]+$", ''
                        .join(validate_default_county) + "a"):
            messages.success(request, mark_safe('There was a problem with \
                default_county <br> Please try again.'))
            return redirect(reverse('home'))
